...
if(facInfo !== "" && facInfo !== undefined && facInfo !== null){
 // alert(200);
var facid = facInfo.facid;
facname = facInfo.facname;
faclga = facInfo.faclga;
...
____________________________________________________________________________________________________
  ____________________________ Works only on facility report page ________________________________
____________________________________________________________________________________________________ */
...
function delStoredComEntries(id){  
    var comStoreList = JSON.parse(localStorage.getItem("comReport-"+FACID));
    if(isEmpty(comStoreList) === false){
      var exist = -1;
    ...
    if(exist !== -1){
      comStoreList[exist].del = 1;
      comStoreList[exist].status = "pending";
...
/*    ______________________________________________________________________________________________
      ______________________________________________________________________________________________
      ______________ To start the process of saving commodity entries to local storage _____________
      ______________________________________________________________________________________________
...
/*____________________________________________________________________________________________
  ______________________ The end of facility-report exclusive scripts ________________________
  ____________________________________________________________________________________________
                                                                                              */

 if(checkLoc("reporting") !== -1){
  getProgress();
  setInterval(getProgress, 30000);
 }

 function getProgress(){
  var arrayLga = ["Akoko North East", "Akoko North West", "Akoko South East", "Akoko South West", "Akure North", "Akure South", "Ese Odo", "Idanre", "Ifedore", "Ilaje", "Ile-oluji/Okeigbo", "Irele", "Odigbo", "Okitipupa", "Ondo East", "Ondo West", "Ose", "Owo"];
 $.get("pages/dbconn.php?upload_progress_lga", function(data){
    if(xhrError(data) === 200){
      ...
      $(".fac-reported").empty();
      $(".report-select-lga").html("<option>Select LGA</option>");

      for(var i = 0; i<18; i++){
      var fids = JSON.stringify(data[i].fid);
    $(".fac-reported").append("<tr><td style='text-align: center'>"+(i+1)+"</td><td><a href='javascript:void(0);' onclick='lgaReport("+i+", "+fids+")'>"+data[i].lga+"</a></td>"+getPercent(data[i].report)+"</tr>");
      total += parseInt(data[i].report[0]);
      curr += parseInt(data[i].report[1]);
      $(".stat-total").html("Report uploaded for "+curr+" out of "+total+" facilities");
      $(".report-select-lga").append("<option pos='"+i+"' data='"+fids+"'>"+arrayLga[i]+"</option>");
  }
      loadKnob(curr, total);
    }
  })
 ...
 $(".report-select-lga").change(function(){
  let lga = $(this).val();
  $(".report-select-lga option").each(function(){
    if($(this).val() === lga){
      let pos = $(this).attr("pos");
      let data = $(this).attr("data");
      lgaReport(pos, JSON.parse(data));
    }
  })
 })

 function getPercent(data){
  total = data[0]; current = data[1]; 
  percent = parseInt((current/total)*100);
  return "<td>"+current+" of "+total+"</td><td>"+percent+"% </td>";
 }

 function loadKnob(current, total){
  percent = parseInt((current/total)*100); 

      $(".knob").val(percent+"%").trigger("change");
      $(".knob").val(percent+"%");
      $(".report-info").html("<span onclick='toggleStat()'>"+current+" of "+total+" facilities");
    }

 function lgaReport(pos, data){
  var arrayLga = ["Akoko North East", "Akoko North West", "Akoko South East", "Akoko South West", "Akure North", "Akure South", "Ese Odo", "Idanre", "Ifedore", "Ilaje", "Ile-oluji/Okeigbo", "Irele", "Odigbo", "Okitipupa", "Ondo East", "Ondo West", "Ose", "Owo"];
  var lga = arrayLga[pos];
  $(".report-select-lga option").each(function(){
    if($(this).val() === lga){
  ...
  $(".repo-lga-name").html(lga);
  var hfs = JSON.parse(localStorage.getItem("hfs"));
  var n = 0;
  var r = 0;
  $(".fac-reported-lga").empty();
  for(var i = 0; i<hfs.length; i++){
    if(hfs[i].lga === lga){
     n++;
    var state = "Not yet reported";
    var bg = "bg-not-reported";
    var repCount = 0;
    var fid = hfs[i].id;
    var cid = "";
    if(isEmpty(data) === false){
        ...
        state = "Reported";
        bg = "";
        repCount = data[a].comNum;
        cid = "onclick = 'facRepoCom(\""+data[a].cids+"\")'";
        r += 1;
      }
    }
      
    $(".fac-reported-lga").append("<tr class='"+bg+"'><td style='text-align: center'>"+n+"</td><td><a href='javascript:void(0);' onclick='facRepo("+fid+", \" "+hfs[i].name+"\", \" "+lga+"\")'>"+hfs[i].name+"</a></td><td>"+repCount+"</td><td>"+state+"</td></tr>");
    }
    else{
    $(".fac-reported-lga").append("<tr class='"+bg+"'><td style='text-align: center'>"+n+"</td><td><a href='javascript:void(0);' onclick='facRepo("+fid+", \" "+hfs[i].name+"\", \" "+lga+"\")'>"+hfs[i].name+"</a></td><td>"+repCount+"</td><td>"+state+"</td></tr>")
    }
  ...
  $(".stat-total-lga").html("Report uploaded for "+r+" out of "+n+" facilities");
  $(".report-card").addClass("hidden");
  $(".report-stat").addClass("hidden");
  $(".report-stat-lga").removeClass("hidden");
}

 function toggleStat(){
  $(".report-card").toggleClass("hidden");
  $(".report-stat").toggleClass("hidden");
 }

 function hideStatLGA(){
  $(".report-stat").removeClass("hidden");
  $(".report-stat-lga").addClass("hidden");
  $(".report-select-lga option").each(function(){
