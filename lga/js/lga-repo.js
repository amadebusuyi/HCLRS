...
$.get("pages/dbconn.php?upload_progress_lga="+userInfo[0].lga, function(data){
    if(xhrError(data) === 200){
      data = JSON.parse(data);

      var fids = JSON.stringify(data.fid);
      let lga = userInfo[0].lga;
      lgaReport(lga, data.fid);
     let total = data.report[0];
     let curr = data.report[1];
 ...
 function toggleStat(){
  ...
  $(".report-stat-lga").toggleClass("hidden");
 }